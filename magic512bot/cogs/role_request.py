...
class RoleRequestModal(discord.ui.Modal, title='Role Request'):
    role_name = discord.ui.TextInput(label='Role Name', placeholder='Enter the role name you want')
    reason = discord.ui.TextInput(label='Reason', style=discord.TextStyle.paragraph, placeholder='Why do you deserve this role?')

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message('Your role request has been submitted!', ephemeral=True)
        await send_role_request_embed(interaction, self.role_name.value, self.reason.value)
...
class ApproveButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"Role {role_name} has been granted to {user.mention}", ephemeral=True)
...
async def send_role_request_embed(interaction: discord.Interaction, role_name: str, reason: str):
