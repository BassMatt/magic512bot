...
def insert_cardloans(session: Session, card_list: list[str], lender: int, borrower: int, borrower_name: str, tag: str = "") -> int:
    """
...
def bulk_return_cardloans(session: Session, lender: int, borrower: int, tag: str = "") -> int:
    """
...
def return_cardloans(session: Session, card_list: list[str], lender: int, borrower: int, tag: str) -> int:
    """
    ...
    for card_name, quantity_to_return in loans_to_return.items():
        ...
        result: List[CardLoan] = session.scalars(card_query_stmt.order_by(CardLoan.created_at)).all()
...
def get_cardloans(session: Session, lender: int, borrower: int, tag: str) -> list[CardLoan]:
    """
